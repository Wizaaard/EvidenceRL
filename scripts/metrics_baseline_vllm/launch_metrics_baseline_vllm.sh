#!/usr/bin/env bash
# ============================================================================
# EvidenceRL BASELINE Enhanced Metrics Launcher (with Judge Selection)
# ============================================================================
# Runs the enhanced metrics pipeline on BASELINE (no-RAG) generation output.
# Same metrics as the RAG version:
...
#
# Usage:
#   ./launch_metrics_baseline_vllm.sh --generation-file <path> --judge <model>
#   ./launch_metrics_baseline_vllm.sh <model_id>                     # Auto-find baseline generation
#   ./launch_metrics_baseline_vllm.sh --help
#
# Examples:
#   # Explicit generation file (recommended)
#   CONDA_ENV=ragcon ./launch_metrics_baseline_vllm.sh \
#       --generation-file generation_baseline_output/gemma-3-4b_v1.0-TEST/gemma-3-4b_baseline_1000-v1.0-TEST.json \
#       --judge llama-3.3-70b \
#       --output-dir metrics_baseline_output/gemma-3-4b_llama70
#
#   # Auto-find generation file for a model
#   CONDA_ENV=ragcon ./launch_metrics_baseline_vllm.sh gemma-3-27b --judge llama-3.3-70b
#
# ============================================================================
...
# Paths - BASELINE directories (separate from RAG)
...
show_help() {
    cat << EOF
${GREEN}EvidenceRL BASELINE Enhanced Metrics Launcher${NC}
${CYAN}Includes F1@k, Macro-F1, Retrieval metrics, Statistical CIs${NC}
${YELLOW}Mode: BASELINE (no-RAG) generation output${NC}

Usage:
  $0 <generation_model> [options]
  $0 --generation-file <path> [options]

Arguments:
  generation_model    Model whose baseline generation to evaluate (e.g., gemma-3-27b)
...
Options:
  ...
  --generation-file <path>     Explicit path to baseline generation JSON
  --output-dir <path>          Output directory (relative paths resolved under metrics_baseline_output/)
...
Examples:
  ...
  # Try different judge with explicit generation file
  CONDA_ENV=ragcon $0 \\
      --generation-file generation_baseline_output/gemma-3-4b_v1.0-TEST/gemma-3-4b_baseline_1000-v1.0-TEST.json \\
      --judge llama-3.3-70b \\
      --output-dir metrics_baseline_output/gemma-3-4b_llama70
...
find_generation_file() {
    ...
    # Common patterns for baseline generation output (using wildcards for patient count flexibility)
    local patterns=(
        "${GENERATION_OUTPUT_DIR}/${model_id}_v${version}/${model_id}_baseline_*-v${version}.json"
        "${GENERATION_OUTPUT_DIR}/${model_id}_v${version}/${model_id}_*-v${version}.json"
        "${GENERATION_OUTPUT_DIR}/${model_id}/${model_id}_baseline_*-v${version}.json"
    )
...
run_enhanced_metrics() {
    ...
    echo -e "${CYAN}Submitting Baseline Enhanced Metrics Job...${NC}"
    ...
    job_id=$(sbatch --parsable \
        ...
        --export=ALL,GENERATION_FILE="${generation_file}",JUDGE_MODEL="${judge_path}",NLI_MODEL="${nli_model}",OUTPUT_JSON="${output_json}",TENSOR_PARALLEL_SIZE="${tensor_parallel}",GPU_MEMORY_UTILIZATION="${gpu_mem}",CONDA_ENV="${CONDA_ENV:-ragcon}" \
        "${SCRIPT_DIR}/metrics_baseline_enhanced.sbatch")
    ...
    if [[ $submit_code -eq 0 ]]; then
        ...
        echo ""
        echo "Monitor: tail -f ${LOG_DIR}/baseline_enhanced_metrics_${job_id}.out"
...
run_judge_comparison() {
    ...
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}Baseline Judge Comparison Mode${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo ""
    echo "Comparing ${#judges_to_compare[@]} judges on baseline output:"
    ...
    # Compare results
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}Baseline Judge Comparison Results${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo "Baseline Judge Comparison Report" > "${comparison_file}"
...
main() {
    ...
    if [[ -z "${generation_file}" ]]; then
        ...
        if ! generation_file=$(find_generation_file "${generation_model}"); then
            echo -e "${RED}Error: Could not find baseline generation file for ${generation_model}${NC}"
            echo "Looked for patterns in:"
            echo "  ${GENERATION_OUTPUT_DIR}/${generation_model}_v1.0/${generation_model}_baseline_*-v1.0.json"
            echo ""
            echo "Use --generation-file to specify the path explicitly."
            exit 1
        fi
    fi
    ...
    echo -e "${GREEN}============================================================${NC}"
    echo -e "${GREEN}EvidenceRL BASELINE Enhanced Metrics${NC}"
    echo -e "${YELLOW}Mode: BASELINE (no-RAG) generation output${NC}"
    echo -e "${GREEN}============================================================${NC}"
    echo ""
