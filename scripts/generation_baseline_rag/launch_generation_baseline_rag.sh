#!/usr/bin/env bash
# ============================================================================
# EvidenceRL Baseline RAG Generation Launcher
# ============================================================================
# Runs generation WITH RAG on the held-out test set (patients 1000-1099).
# This allows comparing RAG vs no-RAG performance on the same patients.
#
# Usage:
#   ./launch_generation_baseline_rag.sh <model_id> [version]
#   ./launch_generation_baseline_rag.sh --list
#   ./launch_generation_baseline_rag.sh --help
#
# Output directory:
...
show_help() {
    echo -e "${GREEN}EvidenceRL Baseline RAG Generation Launcher${NC}"
    echo ""
...
JOB_ID=$(sbatch --parsable --export="${EXPORT_VARS}" "${SCRIPT_DIR}/generation_baseline_rag_master.sbatch")
...
echo "  squeue -u \$USER"
echo "  tail -f ${PROJECT_ROOT}/logs_evidencerl_baseline_rag/master_${JOB_ID}.out"
echo ""
