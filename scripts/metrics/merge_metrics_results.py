#!/usr/bin/env python3
"""Merge metrics results from multiple workers into a single file.

This script:
1. Finds all worker metrics JSON files in input directory
2. Validates structure and sorts by patient_start_idx
3. Merges patient_metrics lists
4. Computes global aggregate metrics
5. Writes merged output JSON

Called by metrics_master.sbatch after all workers complete.
"""
...
def load_worker_files(input_dir: Path, model_id: str) -> List[Dict[str, Any]]:
    """Load and validate all worker JSON files."""
    pattern = f"{model_id}_metrics_worker*.json"
    files = sorted(input_dir.glob(pattern))

    if not files:
        print(f"[Merge] ERROR: No files found matching pattern: {pattern}")
        sys.exit(1)

    print(f"[Merge] Found {len(files)} worker files")
    ...
    for f in files:
        try:
            ...
            if "patient_metrics" not in data:
                print(f"[Merge] WARNING: {f.name} missing 'patient_metrics', skipping")
                continue

            if "config" not in data:
                print(f"[Merge] WARNING: {f.name} missing 'config', skipping")
                continue

            worker_data.append({
                "file": f.name,
                "data": data,
                "start_idx": data["config"].get("patient_start_idx", 0),
                "end_idx": data["config"].get("patient_end_idx", 0),
                "num_patients": len(data["patient_metrics"]),
            })
            print(f"[Merge] Loaded {f.name}: patients [{data['config'].get('patient_start_idx', '?')}:{data['config'].get('patient_end_idx', '?')}] ({len(data['patient_metrics'])} patients)")

        except json.JSONDecodeError as e:
            print(f"[Merge] ERROR: Invalid JSON in {f.name}: {e}")
        except Exception as e:
            print(f"[Merge] ERROR: Failed to load {f.name}: {e}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge metrics results from multiple workers"
    )
    parser.add_argument(
        "--input-dir",
        required=True,
    ...
    parser.add_argument(
        "--model-id",
        required=True,
        help="Model ID to filter worker files",
    )
    parser.add_argument(
        "--version",
        default="1.0",
        help="Version string (default: 1.0)",
    )
    ...
    worker_data = load_worker_files(input_dir, args.model_id)
    ...
    config = worker_data[0]["data"]["config"].copy()
