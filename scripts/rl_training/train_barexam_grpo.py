#!/usr/bin/env python3
"""
GRPO training script for BarExam QA.

Trains a base model with LoRA using online RL with three reward signals:
  R_format:    Valid JSON with answer (A/B/C/D) + reasoning
  R_accuracy:  Exact match on answer letter (no embeddings needed)
  R_grounding: NLI-based evidence grounding (DeBERTa-v3-large)

Uses TRL GRPOTrainer with vLLM for fast generation.
Mirrors the medical train_grpo.py but adapted for MCQ format.

Usage:
    python train_barexam_grpo.py \
        --base-model /path/to/model \
        --dataset /path/to/barexam_grpo_rag_train_prompts.json \
...
def main():
    parser = argparse.ArgumentParser(
        description="GRPO training for BarExam QA",
    ...
    parser.add_argument("--no-vllm", action="store_true",
                        help="Disable vLLM, use transformers for generation")
    ...
    parser.add_argument("--num-iterations", type=int, default=None,
                        help="Policy update iterations per generation batch")
    parser.add_argument("--resume-from-checkpoint", type=str, default=None)
    parser.add_argument("--reward-weight-grounding", type=float, default=None,
                        help="Override grounding reward weight (default: 2.0)")
    ...
    # ── Print config ─────────────────────────────────────────────────
    print("=" * 70)
    print("BAREXAM GRPO TRAINING CONFIGURATION")
    print("=" * 70)
    ...
    # ── Patch vLLM for BitsAndBytes 4-bit compatibility ──────────────
    ...
    # Import BarExam reward functions
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from barexam_grpo_reward import (
        format_reward,
        accuracy_reward,
        grounding_reward,
    ...
    hf_data = Dataset.from_list([
        {
            ...
            "gold_passage": entry["gold_passage"],
    ...
    print(f"  Num iterations/gen:     {num_iters}")
    ...
    # ── Train ────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("STARTING BAREXAM GRPO TRAINING")
    print("=" * 70 + "\n")
    ...
    print("\n" + "=" * 70)
    print("BAREXAM GRPO TRAINING COMPLETE")
...
def _default_config():
    """Default BarExam GRPO training configuration."""
    return {
        "model": {
            "base_model": None,
        ...
        "grpo": {
            "num_generations": 8,
            "max_completion_length": 1024,   # BarExam outputs shorter than medical
            "max_prompt_length": 4096,       # BarExam prompts shorter than medical
            "temperature": 0.7,
            "beta": 0.04,
            "loss_type": "grpo",
            "scale_rewards": "group",
            "epsilon": 0.2,
            "reward_weights": [1.0, 1.0, 2.0],  # format, accuracy, grounding
            "num_iterations": 2,
        },
        "vllm": {
            "use_vllm": True,
