#!/usr/bin/env bash
# ============================================================================
# EvidenceRL Baseline Generation Launcher (No-RAG)
# ============================================================================
# Main entry point for launching distributed baseline (zero-shot) generation.
# This runs generation WITHOUT retrieval - pure LLM inference without RAG.
#
# Usage:
#   ./launch_generation_baseline.sh <model_id> [version]
#   ./launch_generation_baseline.sh --list              # List available models
#   ./launch_generation_baseline.sh --help              # Show help
#
# Examples:
#   ./launch_generation_baseline.sh gemma-3-4b          # Run gemma-3-4b-it
#   ./launch_generation_baseline.sh gemma-3-4b 1.1      # Run with version 1.1
#   ./launch_generation_baseline.sh Llama-3.1-8B        # Run Llama-3.1-8B-Instruct
#
# This script:
...
#
# Output directory:
#   ${PROJECT_ROOT}/../generation_baseline_output
# ============================================================================
...
show_help() {
    echo -e "${GREEN}EvidenceRL Baseline Generation Launcher (No-RAG)${NC}"
    echo ""
    echo "This launcher runs generation WITHOUT retrieval (zero-shot baseline)."
    echo ""
    echo "Usage:"
    echo "  $0 <model_id> [version]    Launch baseline generation for specified model"
    ...
    echo "  PATIENT_OFFSET   Starting patient index (default: 1000, for held-out test set)"
    ...
    echo "  WORKER_TIME      Time limit per worker (default: 02:00:00)"
    echo ""
    echo "Note: 1 worker per 100 patients (auto-calculated from TOTAL_PATIENTS)"
    echo ""
    ...
    echo "  ${PROJECT_ROOT}/../generation_baseline_output"
}
...
mkdir -p "${PROJECT_ROOT}/logs_evidencerl_baseline"

# Conda environment (allow override via environment variable)
CONDA_ENV="${CONDA_ENV:-evidencerl}"

# Baseline test set configuration:
# - Use 100 patients starting at index 1000 (to avoid overlap with training set 0-999)
# - This ensures baseline evaluation is on held-out data
# - 1 worker per 100 patients (simple scaling)
...
echo -e "${GREEN}EvidenceRL Baseline Generation Launcher (No-RAG)${NC}"
echo -e "${GREEN}============================================================${NC}"
echo -e "Mode:           ${YELLOW}BASELINE (zero-shot, no retrieval)${NC}"
...
JOB_ID=$(sbatch --parsable --export="${EXPORT_VARS}" "${SCRIPT_DIR}/generation_baseline_master.sbatch")

echo -e "[launcher] Master job submitted: ${GREEN}${JOB_ID}${NC}"
echo ""
echo "Configuration summary:"
echo "  Mode:               BASELINE (no-RAG)"
...
echo "  squeue -u \$USER"
echo "  tail -f ${PROJECT_ROOT}/logs_evidencerl_baseline/master_${JOB_ID}.out"
echo ""
echo "Output will be saved to:"
echo "  ${PROJECT_ROOT}/../generation_baseline_output/${MODEL_ID}_v${VERSION}/"
