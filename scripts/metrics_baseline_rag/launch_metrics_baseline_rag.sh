#!/usr/bin/env bash
# ============================================================================
# EvidenceRL Baseline RAG Metrics Computation Launcher
# ============================================================================
# Runs metrics computation on BASELINE RAG generation output (with retrieval).
#
# Usage:
#   ./scripts/metrics_baseline_rag/launch_metrics_baseline_rag.sh <model_id> [version]
#   ./scripts/metrics_baseline_rag/launch_metrics_baseline_rag.sh --list
# ============================================================================
...
Computes metrics on BASELINE RAG (with retrieval) generation output.
...
Judge Model: ${JUDGE_MODEL_NAME} (fixed)
...
list_models() {
    echo "Available models for baseline RAG metrics:"
    echo ""
    ...
    for model_id in "${!MODELS[@]}"; do
        ...
        if [ -f "$gen_file" ]; then
            ...
            status="✗ No baseline RAG generation"
        fi
        printf "%-20s %s\n" "$model_id" "$status"
    done
}
...
if [ ! -f "$GENERATION_FILE" ]; then
    echo "ERROR: Baseline RAG generation not found: ${GENERATION_FILE}"
    echo "Run: ./scripts/generation_baseline_rag/launch_generation_baseline_rag.sh ${MODEL_ID}"
    exit 1
fi
...
echo "============================================================"
echo "EvidenceRL Baseline RAG Metrics Computation"
echo "============================================================"
...
JOB_ID=$(sbatch --parsable \
    ...
    "${SCRIPT_DIR}/metrics_baseline_rag_master.sbatch")

echo "Master job submitted: ${JOB_ID}"
echo ""
echo "Monitor: tail -f ${LOG_DIR}/metrics_baseline_rag_master_${JOB_ID}.out"
