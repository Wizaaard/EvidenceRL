#!/usr/bin/env bash
# ============================================================================
# EvidenceRL Baseline Generation Launcher (vLLM Version)
# ============================================================================
# High-performance BASELINE generation (no-RAG) using vLLM.
# Generates diagnoses using only patient context (zero-shot),
# without retrieving evidence from the cardiology knowledge base.
#
# Usage:
#   ./launch_generation_baseline_vllm.sh <model_id> [version]
#   ./launch_generation_baseline_vllm.sh --list              # List available models
#   ./launch_generation_baseline_vllm.sh --help              # Show help
#
# Examples:
#   ./launch_generation_baseline_vllm.sh gemma-3-27b          # Run gemma-3-27b-it
#   ./launch_generation_baseline_vllm.sh gemma-3-27b 1.1      # Run with version 1.1
#
# Output directory:
...
show_help() {
    echo -e "${GREEN}EvidenceRL Baseline Generation Launcher (vLLM Version)${NC}"
    echo -e "${CYAN}High-performance baseline (no-RAG) generation${NC}"
    echo ""
    echo "Usage:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 gemma-3-27b              # Run gemma-3-27b-it baseline"
    ...
    echo ""
    echo "Baseline Mode:"
    echo "  - No document retrieval (zero-shot generation)"
    echo "  - Model relies only on patient context and training knowledge"
    echo "  - Faster than RAG (no embedding, FAISS, or retrieval overhead)"
    echo ""
...
echo -e "${GREEN}============================================================${NC}"
echo -e "${GREEN}EvidenceRL Baseline Generation Launcher (vLLM Version)${NC}"
echo -e "${YELLOW}Mode: BASELINE (no-RAG, zero-shot generation)${NC}"
echo -e "${GREEN}============================================================${NC}"
...
echo -e "Inference:      ${CYAN}vLLM${NC} (no-RAG baseline)"
echo -e "${GREEN}============================================================${NC}"
echo ""

echo "[launcher] Submitting vLLM baseline master job..."
...
JOB_ID=$(sbatch --parsable --export="${EXPORT_VARS}" "${SCRIPT_DIR}/generation_baseline_vllm_master.sbatch")

echo -e "[launcher] Master job submitted: ${GREEN}${JOB_ID}${NC}"
echo ""
echo "Configuration summary:"
echo "  Mode:               BASELINE (no-RAG, zero-shot)"
...
echo "  Inference engine:   vLLM (no-RAG baseline)"
echo ""
echo "Monitor progress:"
echo "  squeue -u \$USER"
echo "  tail -f ${PROJECT_ROOT}/logs_evidencerl_baseline/generation_baseline_vllm_master_${JOB_ID}.out"
echo ""
